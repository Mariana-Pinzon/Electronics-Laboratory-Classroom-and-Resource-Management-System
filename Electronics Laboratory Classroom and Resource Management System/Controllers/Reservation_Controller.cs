...
namespace Electronics_Laboratory_Classroom_and_Resource_Management_System.Controllers
{
    ...
    public class Reservation_Controller : ControllerBase
    {
        ...
        public async Task<ActionResult> CreateReservation(int User_ID, int Laboratory_ID, [FromQuery] List<int> Reservation_Equipments, DateOnly Reservation_date, TimeOnly Start_time, TimeOnly End_time, int StatusR_ID, [FromBody] Reservation reservation)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            await _reservationService.CreateReservationAsync(User_ID, Laboratory_ID, Reservation_Equipments, Reservation_date, Start_time, End_time, StatusR_ID, reservation);
